...
class Xian_CellGuideMaskResize
{
    ...
    private:
        ...
        sensor_msgs::ImagePtr tl_mask_msg;
        sensor_msgs::ImagePtr tr_mask_msg;
        sensor_msgs::ImagePtr bl_mask_msg;
        sensor_msgs::ImagePtr br_mask_msg;
        ...
        void command_callback(const xian_msg_pkg::xian_cell_guide_maskConstPtr& data)
        {
            ...
            cv::resize(tl_mask, tl_mask, cv::Size(crop_w, crop_h), 2);
            cv::resize(tr_mask, tr_mask, cv::Size(crop_w, crop_h), 2);
            cv::resize(bl_mask, bl_mask, cv::Size(crop_w, crop_h), 2);
            cv::resize(br_mask, br_mask, cv::Size(crop_w, crop_h), 2);
            ...
            tl_mask = zpmc::zpmc_MaskBin0_255(tl_mask, 80);
            tr_mask = zpmc::zpmc_MaskBin0_255(tr_mask, 80);
            bl_mask = zpmc::zpmc_MaskBin0_255(bl_mask, 80);
            br_mask = zpmc::zpmc_MaskBin0_255(br_mask, 80);
            ...
            tl_mask_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", tl_mask).toImageMsg();
            tr_mask_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", tr_mask).toImageMsg();
            bl_mask_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", bl_mask).toImageMsg();
            br_mask_msg = cv_bridge::CvImage(std_msgs::Header(), "bgr8", br_mask).toImageMsg();
            ...
            cell_guide_masks_resize.tl_mask_resize = *tl_mask_msg;
            cell_guide_masks_resize.tr_mask_resize = *tr_mask_msg;
            cell_guide_masks_resize.bl_mask_resize = *bl_mask_msg;
            cell_guide_masks_resize.br_mask_resize = *br_mask_msg;
            ...
            cell_guide_masks_resize.tl_cell_guide_crop_tl_x = data->tl_cell_guide_crop_tl_x;
            cell_guide_masks_resize.tl_cell_guide_crop_tl_y = data->tl_cell_guide_crop_tl_y;
            cell_guide_masks_resize.tr_cell_guide_crop_tl_x = data->tr_cell_guide_crop_tl_x;
            cell_guide_masks_resize.tr_cell_guide_crop_tl_y = data->tr_cell_guide_crop_tl_y;
            cell_guide_masks_resize.bl_cell_guide_crop_tl_x = data->bl_cell_guide_crop_tl_x;
            cell_guide_masks_resize.bl_cell_guide_crop_tl_y = data->bl_cell_guide_crop_tl_y;
            cell_guide_masks_resize.br_cell_guide_crop_tl_x = data->br_cell_guide_crop_tl_x;
            cell_guide_masks_resize.br_cell_guide_crop_tl_y = data->br_cell_guide_crop_tl_y;
