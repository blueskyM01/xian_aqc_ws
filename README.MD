...
- If do not have configured docker, docker preparation as follow
    ...
    - Start docker (`reciever side`, docker visualization)
        ...
        - run docker image
            ```
            sudo docker run --name xian_aqc -itd --log-opt max-size=50m --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" -v /home/nvidia/yangjianbing/:/root/code --net host -e NVIDIA_DRIVER_CAPABILITIES=compute,utility --env="DISPLAY" --env="QT_X11_NO_MITSHM=1" --gpus all --shm-size="12g" --restart=always nvcr.io/nvidia/l4t-jetpack:r35.4.1
            ```
        - 注意：
    ...
    - Start docker (`sender side`, docker visualization)
        ...
        - run docker image 
            ```
            sudo docker run --name xian_aqc -itd --log-opt max-size=50m --device /dev/video0 --device /dev/video1 --device /dev/video2 --device /dev/video3 --volume="/tmp/.X11-unix:/tmp/.X11-unix:rw" -v /home/nvidia/yangjianbing/:/root/code -p 2810::22 -p 2812:2812 -p 8000-8030:8000-8030 -p 2840:2840 -e NVIDIA_DRIVER_CAPABILITIES=compute,utility --env="DISPLAY" --env="QT_X11_NO_MITSHM=1" --gpus all --shm-size="12g" --restart=always nvcr.io/nvidia/l4t-jetpack:r35.4.1
